...
#     # Annotators (keep it simple & stable)
...
#             hres = h_estimate = h_est.estimate(kp)
            

#             transformer = None
#             if hres.H is not None:
#                 transformer = h_est.make_transformer(hres.H)

#             # 5) Project to pitch coords (if H available)
#             pitch_xy_tracks = np.zeros((len(tracks), 2), dtype=np.float32)
#             pitch_xy_ball = np.zeros((len(ball_det), 2), dtype=np.float32)

#             if transformer is not None:
#                 pitch_xy_tracks = project_anchors_to_pitch(transformer, tracks, anchor=sv.Position.BOTTOM_CENTER)
#                 pitch_xy_ball = project_anchors_to_pitch(transformer, ball_det, anchor=sv.Position.BOTTOM_CENTER)
...
#                     "x_m": float(pitch_xy_tracks[i, 0]) if transformer is not None else 0.0,
#                     "y_m": float(pitch_xy_tracks[i, 1]) if transformer is not None else 0.0,
#                     "homography_ok": bool(hres.ok),
#                     "kp_used": int(hres.n_points),
#                     "inlier_ratio": float(hres.inlier_ratio),
#                     "reproj_err": float(hres.reproj_err),
#                 }
#                 # attach team_id only for players
#                 if team_ids is not None and int(tracks.class_id[i]) == PLAYER_ID:
#                     # players_tr is a subset, so we need alignment; simplest is re-compute mapping:
#                     # Here we do quick mapping by filtering order:
#                     pass
#                 csvw.write_row(row)
...
#                     "x1": x1, "y1": y1, "x2": x2, "y2": y2,
#                     "x_m": float(pitch_xy_ball[j, 0]) if transformer is not None else 0.0,
#                     "y_m": float(pitch_xy_ball[j, 1]) if transformer is not None else 0.0,
#                     "homography_ok": bool(hres.ok),
#                     "kp_used": int(hres.n_points),
#                     "inlier_ratio": float(hres.inlier_ratio),
#                     "reproj_err": float(hres.reproj_err),
